...
| Judge model `qwen3:4b-instruct` (LLM_ADDRESSEE_MODEL unset) | Measured on the mini over 40 made-up example conversations (20 aimed at Lu, 20 not): `qwen3:1.7b` gave an unclear answer on 40/40 — it is a thinking model and a 3-token answer cap is used up by its reasoning; switching thinking off needs Ollama's native API, not the OpenAI-compatible one the bot uses. `qwen3:4b-instruct` median 2.29s, p90 3.50s per judge call. |
